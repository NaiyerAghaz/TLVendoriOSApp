//
//  ApiCalendarDataResponseModel.swift
//  TotalVendor
//
//  Created by Darrin Brooks on 29/10/21.
//

import Foundation

struct ApiCalendarDataResponseModel : Codable {
    let vENDORSCHEDULEDATA : [ApiCalendarDataVendorScheduleData]?
    let appointmentStatus : [ApiCalendarDataAppointmentStatus]?

    enum CodingKeys: String, CodingKey {

        case vENDORSCHEDULEDATA = "VENDORSCHEDULEDATA"
        case appointmentStatus = "AppointmentStatus"
    }

    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        vENDORSCHEDULEDATA = try values.decodeIfPresent([ApiCalendarDataVendorScheduleData].self, forKey: .vENDORSCHEDULEDATA)
        appointmentStatus = try values.decodeIfPresent([ApiCalendarDataAppointmentStatus].self, forKey: .appointmentStatus)
    }
...
struct ApiCalendarDataVendorScheduleData : Codable {
    let filecode : String?
...
struct ApiCalendarDataAppointmentStatus : Codable {
    let id : Int?
    let code : String?
    let value : String?
    let type : String?
